...
public class FileSelector {
    ...
    synchronized Long
        ...
        final Long fileNum = calculator.getBestFile(
            fileSummaryMap, false /*forceCleaning*/,
            null /*lowUtilizationFiles*/, false /*isBacklog*/,
            true /*isProbe*/, new HashSet<Long>(excludeFilesForCorrection));

        /* Rotate excluded files to include newly selected file. */
        if (fileNum != null) {
            excludeFilesForCorrection.addLast(fileNum);
            while (excludeFilesForCorrection.size() >
                   MAX_EXCLUDE_FILES_FOR_CORRECTION) {
                excludeFilesForCorrection.removeFirst();
            }
        }
