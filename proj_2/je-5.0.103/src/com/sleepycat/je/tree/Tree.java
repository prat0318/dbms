...
public final class Tree implements Loggable {
    ...
    public BIN getNextBin(BIN bin,
        ...
        return (BIN) getNextIN(bin, true, cacheMode);
    }
    ...
    public BIN getPrevBin(BIN bin,
        ...
        return (BIN) getNextIN(bin, false, cacheMode);
    }
    ...
    public IN getNextIN(IN prevIn, boolean forward, CacheMode cacheMode) {
         ...
         * reason is that the IN may get split while finding the next IN so
        ...
        if (prevIn.getNEntries() == 0) {
            idKey = prevIn.getIdentifierKey();
        } else if (forward) {
            idKey = prevIn.getKey(prevIn.getNEntries() - 1);
        } else {
            idKey = prevIn.getKey(0);
        }
            ...
            while (true) {
                ...
                if (moreEntriesThisIn) {
                    ...
                    if (nextIN.getLevel() == targetLevel) {
                        ...
                        final IN ret = searchSubTree(nextIN, null,
                                                     (forward ?
                                                      SearchType.LEFT :
                                                      SearchType.RIGHT),
                                                     targetLevel,
    ...
    public IN search(byte[] key,
        ...
        if (rootIN != null) {
            return searchSubTree(rootIN, key, searchType, -1 /*targetLevel*/,
                                 binBoundary, cacheMode, searchComparator);
        } else {
            return null;
        }
    }
        ...
        for (int i = 0; i < 2; i++) {
            try {
                return searchSubTreeInternal
                    (parent, key, searchType, targetLevel, binBoundary,
     ...
     *
     * Splits cause INs to be logged in all ancestors up to and including the
     * root.  This is so that recovery is guaranteed never to have to do a
     * split when replaying INs.  For more explanation, see
     * RecoveryManager.recoverChildIN.
     *
     * @param parent is the root IN.
     *
     ...
     */
    public IN getRootINRootAlreadyLatched(CacheMode cacheMode,
                                          boolean exclusive) {
        if (!rootExists()) {
            return null;
        }
