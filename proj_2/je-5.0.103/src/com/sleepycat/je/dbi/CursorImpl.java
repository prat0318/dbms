...
public class CursorImpl implements Cloneable {
    ...
    /**
     * An alternative to performCacheEviction that is called when a non-cloning
     * cursor is used, when the cursor has a valid position and an advancing
     * operation (next/prev/skip) is about to be performed. The cursor cannot
     * be reset as it would be if the operation were a search, for example. Yet
     * when EVICT_LN is configured we should evict the LN at the current
     * position before changing the position. [#21973]
     * <p>
     * Note that EVICT_BIN is not supported for a non-cloning cursor.  We would
     * have to call removeCursor before the BIN eviction, and then our position
     * would be unstable.  Even if this were possible, since we can't predict
     * the new position we would evict the BIN at every operation, even when
     * the BIN remained the same.
     * <p>
     * It might be possible to support MAKE_COLD here, but more thought and
     ...
     */
    public Pair<OperationStatus, Boolean> put(
    ...
    public OperationStatus putLN(byte[] key,
        ...
        final Pair<OperationStatus, Boolean> result = putInternal(
     ...
     * @return pair of status and 'inserted' boolean.
     */
    private Pair<OperationStatus, Boolean> putInternal(
        final LN ln,
        final byte[] key,
        final boolean allowOverwrite,
        final DatabaseEntry overwriteData,
        final DatabaseEntry returnOldData,
        final DatabaseEntry returnNewData,
        final ReplicationContext repContext) {
        ...
        try {
            ...
            if (insertNewSlot(ln, key, repContext)) {
                ...
                return new Pair<OperationStatus, Boolean>(
            ...
            if (!allowOverwrite) {
                return new Pair<OperationStatus, Boolean>(
                    OperationStatus.KEYEXIST, false);
            }
             ...
             */
            final OperationStatus status = putCurrentAlreadyLatchedAndLocked(
                key, overwriteData, null /*foundKey*/, returnOldData,
                returnNewData, slotInfo.lockStanding, repContext);
            return new Pair<OperationStatus, Boolean>(status, false);
        } finally {
            releaseBIN();
        }
    }
     ...
     * match is found, and the record will be locked by this method.</p>
     *
     * <p>If SET is not specified, this method does not lock the record.  The
     * caller is expected to call getCurrent to performing locking.</p>
     *
        ...
        try {
            ...
            if (bin != null) {
                ...
                if (index >= 0) {
                    ...
                    /*
                     * For an exact search we must always try locking, even if
                     * the record is deleted, since the caller will not call
                     * getCurrent. [#22892]
                     */
                    if (exactSearch) {
                        LockStanding lockStanding = lockLN(lockType);
     ...
     *
     * TODO: check for index GTE nEntries?  See similar check in fetchCurrent.
     */
