 ...
 * The writing of data to the log is serialized via the logWriteLatch.
...
public class LogManager {
    ...
    public void multiLog(LogItem[] itemArray, LogContext context)
        ...
        try {
            ...
            while (lqe != null) {
                ...
                logItems(new LogItem[] { item }, lqeContext);
             ...
             */
            logItems(itemArray, context);

            if (context.fsyncRequired || context.flushRequired) {
                /* Flush log buffer and fsync */
                grpManager.sync(context.fsyncRequired);
            } else if (context.switchedLogBuffer) {
                /*
             ...
             */
            if (!envImpl.isValid() ||
                FileManager.continueAfterWriteException()) {
                throw e;
            }
    ...
    /**
     *
     * The work, other than exception handling.
     */
    private void logItems(LogItem[] itemArray, LogContext context)
        ...
        for (LogItem item : itemArray) {
            LogWriteInfo lwi = null;
            flushRequired =
                (item == lastItem) &&
                context.flushRequired &&
                !context.fsyncRequired;
            forceNewLogFile = (item == firstItem) && context.forceNewLogFile;
            lwi = serialLog(item,
                            context,
                            forceNewLogFile,
                            flushRequired,
            ...
            if (lwi != null) {
                 ...
                 */
                item.buffer =
                    item.header.addPostMarshallingInfo(item.buffer,
                                                       lwi.fileOffset,
                                                       lwi.vlsn);
        ...
        updateObsolete(context, tracker);
    }
    ...
    */
    private LogWriteInfo serialLog(LogItem item,
                                             LogContext context,
                                             boolean forceNewLogFile,
                                             boolean flushRequired,
     ...
     * @throws IOException
     */
    private LogWriteInfo serialLogWork(LogItem item,
                                         LogContext context,
                                         boolean forceNewLogFile,
                                         boolean flushRequired,
                                         UtilizationTracker tracker)
        ...
        try {
            ...
            if (lastLogBuffer != prevLogBuffer) {
                context.switchedLogBuffer = true;
            }
            prevLogBuffer = lastLogBuffer;

            fileOffset = fileManager.getPrevEntryOffset();

            if (item.repContext.getClientVLSN() != null ||
                item.repContext.mustGenerateVLSN()) {

                if (item.repContext.mustGenerateVLSN()) {
                    vlsn = envImpl.bumpVLSN();
                } else {
                    vlsn = item.repContext.getClientVLSN();
                }
            }
         ...
         */
        item.entry.postLogWork(item.header, currentLsn, vlsn);
    ...
    public void updateObsolete(LogContext context,
                               UtilizationTracker tracker) {
        synchronized (logWriteLatch) {
