...
public class Replay {
    ...
    /**
     * Reinitialize Replay for a new feeder replica connection. This method
     * must be called to ensure that the channel  and protocol object is
     * correctly set up before entries are replayed via the replayEntry
     * method.
     */
    @SuppressWarnings("hiding")
    public void reinit(NamedChannel replicaFeederChannel,
                       Protocol protocol) {
    ...
    private void updateCommitStats(final boolean needsAck,
        ...
        /*
         * Tally the lag between master and replica commits, ignoring cases
         * where clock skew made the lag appear negative.
         */
        if (replicaCommitTimeMs > masterCommitTimeMs) {
            totalCommitLagMs.add(replicaCommitTimeMs - masterCommitTimeMs);
        }
    }
     ...
     * <p>
     * The channel and protocol object should have been initialized via
     * a call to the reinit method.
     *
     * @see #reinit
     */
    ...
    private void applyLN(final ReplayTxn repTxn,
        ...
        try {
            ...
            if (ln.isDeleted()) {
                ...
                status = DbInternal.searchForReplay(cursor,
                                                    replayKeyEntry,
                                                    delDataEntry,
     ...
     *
     * @param nowMs the time at the readin of the log entry
     */
    ...
    private class GroupCommit {
        ...
        private final void flushPendingAcks(long nowNs)
            ...
            for (int i=0; i < nPendingAcks; i++) {
                final long txnId = pendingCommitAcks[i];
