 ...
 * <p>
 * For <em>transactional</em> non-replicated databases, special configuration
 * of user supplied transactions may be needed.  If only non-replicated
 * databases are accessed by a transaction, it is normally desirable to
 * configure the transaction so that group consistency checks are not performed
 * and acknowledgements from other nodes are not required.  This allows the
 ...
 * is used (no explicit transaction is supplied) with a transactional,
 * non-replicated database, the internal auto-commit transaction is
 * automatically configured as described.  A user-supplied transaction may be
 * configured accordingly as follows.
 * <ul>
 * <li>Consistency checks can be disabled by calling
 * {@link com.sleepycat.je.TransactionConfig#setConsistencyPolicy} with
 * {@link NoConsistencyRequiredPolicy#NO_CONSISTENCY}.</li>
 * <li>Acknowledgement checks can be disabled by calling
 * {@link com.sleepycat.je.TransactionConfig#setDurability} with
 * {@link com.sleepycat.je.Durability.ReplicaAckPolicy#NONE}.</li>
 * </ul>
 * <p>
 * Although unusual, a single transaction can be used to perform read and write
 * operations for a combination of replicated and non-replicated databases.  As
 * would be expected, writing to a replicated database is only allowed on a
 * master and only the changes to the replicated databases are included in the
 * replication stream.  For such transactions, consistency and durability
 * should be configured as usual for accessing a replicated database.
 *
 * @see Environment
...
public class ReplicatedEnvironment extends Environment {
     ...
     * If the group has no current master, creation of a handle will trigger an
     * election to determine whether this node will participate as a Master or
     * a Replica.
     * <p>
     ...
     * node must always specify one or more active helper nodes via the
     * {@link ReplicationConfig#setHelperHosts(String)} method, or via the the
     ...
     * elections to wait until all nodes can vote. By ensuring that all the
     * nodes can vote, the best possible node is chosen to be the master at
     * group startup.
     * <p>
     ...
     * {@link ReplicationConfig#ENV_UNKNOWN_STATE_TIMEOUT} is zero (the default
     * value) and the node cannot join the group in the time period specified
     * by {@link ReplicationConfig#ENV_SETUP_TIMEOUT}.
     *
     ...
     * consistent with the master as of the time the handle was created.</p>
     *
     ...
     * made consistent within the timeout specified by {@link
     * ReplicationConfig#ENV_CONSISTENCY_TIMEOUT}
     *
     * @throws UnknownMasterException if the
     * {@link ReplicationConfig#ENV_UNKNOWN_STATE_TIMEOUT} has a zero
     * value and the node cannot join the group in the time period specified by
     * the {@link ReplicationConfig#ENV_SETUP_TIMEOUT} property.
     *
     * @throws EnvironmentLockedException when an environment cannot be opened
     * for write access because another process has the same environment open
     * for write access.  <strong>Warning:</strong> This exception should be
     ...
     * testing purposes.
     *
     * TODO: This should be renamed, at a convenient time. It tests for both a
     * closed environment as well as an invalid environment.
     *
     * @return true if this handle has been closed.
     */
    boolean isClosed() {
        return (repImpl == null) || repImpl.isClosed() ||  repImpl.isInvalid();
    }
    ...
    public void printStartupInfo(PrintStream out) {
        ...
        envImpl.getStartupTracker().displayStats(out, Phase.TOTAL_JOIN_GROUP);
    }
