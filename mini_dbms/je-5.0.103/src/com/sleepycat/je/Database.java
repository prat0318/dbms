...
public class Database implements Closeable {
    ...
    private void
        ...
        if (envImpl != null) {
            envImpl.getSecondaryAssociationLatch().acquireExclusive();
        }
        try {
            ...
            if (envImpl != null) {
                envImpl.getSecondaryAssociationLatch().release();
            }
        }
    }
    ...
    public boolean populateSecondaries(final DatabaseEntry key,
        ...
        try {
            ...
            trace(Level.FINEST, "populateSecondaries", null, key,
                  null, null);
    ...
    private boolean populateSecondariesInternal(final Cursor cursor,
        ...
        final LockMode scanMode = LockMode.DEFAULT;
        OperationStatus searchStatus;
        if (key.getData() == null) {
            /* Start at first key. */
            searchStatus = cursor.position(key, Cursor.NO_RETURN_DATA,
                                           scanMode, true);
        } else {
            /* Resume at key last processed. */
            searchStatus = cursor.search(key, Cursor.NO_RETURN_DATA, scanMode,
        ...
        while (searchStatus == OperationStatus.SUCCESS) {
            ...
            if (anySecondaries) {
                /* TODO: use RMW for compatibility with new locking order? */
                if (cursor.getCurrent(key, data, LockMode.DEFAULT) ==
                    OperationStatus.SUCCESS) {
                    for (final SecondaryDatabase secDb : secondaries) {
                        if (secDb.isIncrementalPopulationEnabled()) {
                            secDb.updateSecondary(locker, null, key, null,
                                                  data);
                        }
                    }
                }
            }
            searchStatus = cursor.retrieveNext(key, Cursor.NO_RETURN_DATA,
                                               scanMode, GetMode.NEXT);
        }
        return false;
    }
    ...
    OperationStatus deleteInternal(final Locker locker,
        ...
        try {
            ...
            try {
                ...
                /*
                 * Fetch old data with RMW to avoid lock upgrades.  Do not
                 * fetch old data unless needed for secondaries.  And if we
                 * don't need the data, use dirty read to enable the
                ...
                OperationStatus searchStatus =
                    cursor.search(key, oldData, lockMode, SearchMode.SET);
                ...
                while (searchStatus == OperationStatus.SUCCESS) {
                    ...
                    /*
                     * Enforce foreign key constraints before secondary
                     * updates, so that ForeignKeyDeleteAction.ABORT is applied
                     * before deleting the secondary keys.
                     */
                    ...
                    /*
                     * Update secondaries before deletion, so that a primary
                     * record always exists while secondary keys refer to it.
                     * This is relied on by secondary read-uncommitted.
                     */
                    if (secondaries != null) {
                        for (final SecondaryDatabase secDb : secondaries) {
