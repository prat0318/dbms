...
public class Cursor implements ForwardCursor {
    ...
    OperationStatus deleteInternal(final ReplicationContext repContext) {
        ...
        try {
            ...
            if (needOldData || hasAssociations) {
                final OperationStatus status =
                    getCurrentInternal(key, oldData, LockMode.RMW);
     ...
     */
    OperationStatus putForReplay(final DatabaseEntry key,
                                 final DatabaseEntry data,
                                 final LN ln,
                                 final PutMode putMode,
     ...
     *
     * @param key may be null if putMode is CURRENT, must be non-null
     * otherwise.
     *
    ...
    private OperationStatus
        ...
        try {
            /*
             * Determine replacement key before getting current key using
             * an effective dirty-read below.
             */
            final byte[] replaceKey =
                ((putMode == PutMode.CURRENT) && (key != null)) ?
                Key.makeKey(key) :
                null;

            /*
             * If the passed key is null, it must be fetched now if there are
             * secondaries, to determine whether the old data is also needed.
             * If there are no secondaries but there are triggers, the key is
             * returned by CursorImpl.putCurrent.
             */
            if (key == null) {
                assertState(putMode == PutMode.CURRENT);
                if (hasAssociations || hasUserTriggers) {
                    key = new DatabaseEntry();
                    if (hasAssociations) {
                        key.setData(cursorImpl.getCurrentKey());
            ...
            /* Request old data from put operation, if needed. */
            DatabaseEntry oldData = needOldData ? new DatabaseEntry() : null;

            /*
             * If passed data is partial, ask the put operation to return a
             * copy after resolving the partial data. No copy is needed if
             * partial data is not passed. [#16932]
             */
            final DatabaseEntry returnNewData;
            final DatabaseEntry newData;
            if (data.getPartial() && (hasAssociations || hasUserTriggers)) {
            ...
            if (secondaries != null) {
                for (final SecondaryDatabase secDb : secondaries) {
                    if (inserted || secDb.updateMayChangeSecondary()) {
                        secDb.updateSecondary(locker, null, key, oldData,
                                              newData);
                    }
                }
            }
            if (hasUserTriggers) {
                TriggerManager.runPutTriggers(locker, dbImpl, key,
                                              oldData, newData);
            }
    ...
    /**
     * Performs the put operation but does not notify triggers or perform
     * secondary updates.  Prevents phantoms.
     *
     ...
     */
    private Pair<OperationStatus, Boolean>
     ...
     * swap cursors.  Does not notify triggers or perform secondary updates.
    ...
    private Pair<OperationStatus, Boolean>
        ...
        try {
            /* Perform operation. */
            final Pair<OperationStatus, Boolean> result = dup.put(
                key, data, ln, putMode, returnOldData, returnNewData,
                repContext);
            status = result.first();
    ...
    OperationStatus readPrimaryAfterGet(final Database priDb,
         ...
         */
        final Locker locker = cursorImpl.getLocker();
        final Cursor cursor = new Cursor(priDb, locker, null,
        ...
        try {
             ...
             */
            if (readUncommitted && checkForPrimaryUpdate(key, pKey, data)) {
                return OperationStatus.KEYEMPTY;
            }
    ...
    private CursorImpl beginMoveCursor(final boolean addCursor,
        ...
        if (nonCloning) {
            cursorImpl.setCacheMode(cacheMode);
            if (addCursor) {
                cursorImpl.performCacheEvictionBeforeAdvance();
                cursorImpl.criticalEviction();
            } else {
                cursorImpl.reset();
            }
