...
public class EnvironmentStats implements Serializable {
    ...
    public Collection<StatGroup> getStatGroups() {
        return Arrays.asList(cacheStats, ckptStats, cleanerStats, logStats,
                             lockStats, envImplStats, incompStats,
                             throughputStats);
    }
     ...
     * Internal use only.
     */
    public void setCacheStats(StatGroup clonedCacheStats) {
        cacheStats = clonedCacheStats;
    }
     ...
     * is unknown.  Used to calculate corrected utilization.
     *
     * <p>{@link Float#NaN} is returned when a correction factor is not
     * applicable because all record sizes have not yet been counted or enough
     * data has not yet been collected to determine a representative
     * average.</p>
     *
     * <p>When deletions and updates are performed and the record (LN) is not
     * in cache, the size of the obsolete record in the log is unknown and
     * estimated log utilization may be incorrect.  The correction factor is
     * determined when true utilization is calculated as log files are
     * cleaned.</p>
     */
    public float getLNSizeCorrectionFactor() {
        return cleanerStats.getFloat(CLEANER_LN_SIZE_CORRECTION_FACTOR);
    }
     ...
     *
     * <p>When utilization may be incorrect (see {@link
     * #getCorrectedAvgLNSize}) and no log cleaning occurs because the
     * estimated utilization is higher than the configured utilization
     * threshold, a cleaner run is made to probe utilization and determine the
     * correction factor. This run is read-only and does not clean or delete
     * the log file; it only calculates utilization.</p>
     */
    public long getNCleanerProbeRuns() {
        return cleanerStats.getLong(CLEANER_PROBE_RUNS);
    }
